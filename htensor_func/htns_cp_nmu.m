function [P,Uinit] = htns_cp_nmu(X,R,opts)
...
if iscell(init)
    ...
    elseif strcmp(init,'nvecs') || strcmp(init,'eigs') 
        ...
        for n = dimorder(1:end)
            ...
            if (k < R)
              Uinit{n} = [Uinit{n} rand(X.modes(n),R-k)]; 
            end
        end
    else
...
for iter = 1:maxiters
    ...
    for n = dimorder(1:end)
        ...
        tmp = htns_coo_mttkrp(X,U,n) + epsilon;
    ...
    P = ktensor(U);
    normresidual = sqrt( normX^2 + norm(P)^2 - 2 * htns_ktns_innerprod(X,P) );
